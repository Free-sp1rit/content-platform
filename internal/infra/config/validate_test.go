...
func TestAuthConfigValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "access TTL below one second", mutate: func(c *AuthConfig) { c.AccessTokenTTL = 500 * time.Millisecond }, wantErr: "AUTH_ACCESS_TOKEN_TTL must be at least one second"},
		{name: "access TTL not whole seconds", mutate: func(c *AuthConfig) { c.AccessTokenTTL = 1500 * time.Millisecond }, wantErr: "AUTH_ACCESS_TOKEN_TTL must use whole-second precision"},
		...
		{name: "refresh TTL below one second", mutate: func(c *AuthConfig) { c.RefreshTokenTTL = 500 * time.Millisecond }, wantErr: "AUTH_REFRESH_TOKEN_TTL must be at least one second"},
		{name: "refresh TTL not whole seconds", mutate: func(c *AuthConfig) { c.AccessTokenTTL = time.Second; c.RefreshTokenTTL = 1500 * time.Millisecond }, wantErr: "AUTH_REFRESH_TOKEN_TTL must use whole-second precision"},
